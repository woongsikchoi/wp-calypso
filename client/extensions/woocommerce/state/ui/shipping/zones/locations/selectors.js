...
const getCurrentZoneLocationsAndEdits = ( state, siteId ) => {
	...
	return {
		locations: getRawShippingZoneLocations( state, siteId )[ zone.id ],
		edits: getShippingZonesEdits( state, siteId ).currentlyEditingChanges.locations,
	};
};
...
const getCountriesOwnedByOtherZone = ( state, siteId ) => {
	const countries = {};
	const currentZone = getCurrentlyEditingShippingZone( state, siteId );
	forIn( getRawShippingZoneLocations( state, siteId ), ( { country }, zoneId ) => {
		if ( currentZone.id === zoneId ) {
			return;
		}
...
const getShippingZoneLocationsWithEdits = ( state, siteId ) => {
	const locationsAndEdits = getCurrentZoneLocationsAndEdits( state, siteId );
	if ( ! locationsAndEdits ) {
		return null;
	}
	const { locations, edits } = locationsAndEdits;
	if ( edits.pristine ) {
		return locations;
	}
	...
	getContinents( state, siteId ).forEach( ( { code: continentCode, name: continentName } ) => {
		...
		getCountries( state, continentCode, siteId ).forEach( ( { code: countryCode, name: countryName } ) => {
			locationsList.push( {
				...
				disabled: Boolean( forbiddenCountries[ countryCode ] ),
