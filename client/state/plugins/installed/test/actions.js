...
describe( 'actions', () => {
	...
	describe( '#updatePlugin()', () => {
		...
		it( 'should dispatch plugin update request success action when request completes', () => {
			const response = updatePlugin(
				site.ID,
				{ slug: 'jetpack', id: 'jetpack/jetpack', update: {} },
			)( spy );
			return response.then( () => {
				expect( spy ).to.have.been.calledWith( {
	...
	describe( '#enableAutoupdatePlugin()', () => {
		...
		it( 'should dispatch plugin enable autoupdate request success action when request completes', () => {
			const response = enableAutoupdatePlugin(
				site.ID,
				{ slug: 'akismet', id: 'akismet/akismet' },
			)( spy );
			return response.then( () => {
				expect( spy ).to.have.been.calledWith( {
	...
	describe( '#disableAutoupdatePlugin()', () => {
		...
		it( 'should dispatch plugin disable autoupdate request success action when request completes', () => {
			const response = disableAutoupdatePlugin(
				site.ID,
				{ slug: 'akismet', id: 'akismet/akismet' },
			)( spy );
			return response.then( () => {
