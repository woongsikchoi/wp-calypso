		...
		case JETPACK_CONNECT_CHECK_URL_RECEIVE:
			if ( action.url === state.url ) {
				return Object.assign(
					{},
					state,
					{ isFetching: false, isFetched: true, data: action.data },
				);
			}
			return state;
	...
	switch ( action.type ) {
		...
		case JETPACK_CONNECT_SSO_VALIDATION_ERROR:
			return Object.assign(
				{},
				state,
				{ isValidating: false, validationError: action.error, nonceValid: false },
			);
		...
		case JETPACK_CONNECT_SSO_AUTHORIZE_SUCCESS:
			return Object.assign(
				{},
				state,
				{ isAuthorizing: false, authorizationError: false, ssoUrl: action.ssoUrl },
			);
		case JETPACK_CONNECT_SSO_AUTHORIZE_ERROR:
			return Object.assign(
				{},
				state,
				{ isAuthorizing: false, authorizationError: action.error, ssoUrl: false },
			);
	}
	return state;
}
