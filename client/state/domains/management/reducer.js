...
export const requesting = createReducer( false, {
	[ DOMAIN_MANAGEMENT_CONTACT_DETAILS_CACHE_REQUEST ]: state => ( {
		...state,
		_contactDetailsCache: true,
	} ),
	[ DOMAIN_MANAGEMENT_CONTACT_DETAILS_CACHE_REQUEST_SUCCESS ]: state => ( {
		...state,
		_contactDetailsCache: false,
	} ),
	[ DOMAIN_MANAGEMENT_CONTACT_DETAILS_CACHE_REQUEST_FAILURE ]: state => ( {
		...state,
		_contactDetailsCache: false,
	} ),
	[ DOMAIN_MANAGEMENT_WHOIS_REQUEST ]: ( state, { domain } ) => ( { ...state, [ domain ]: true } ),
	[ DOMAIN_MANAGEMENT_WHOIS_REQUEST_SUCCESS ]: ( state, { domain } ) => ( {
		...state,
		[ domain ]: false,
	} ),
	[ DOMAIN_MANAGEMENT_WHOIS_REQUEST_FAILURE ]: ( state, { domain } ) => ( {
		...state,
		[ domain ]: false,
	} ),
} );
...
export const items = createReducer(
	{},
	{
		[ DOMAIN_MANAGEMENT_CONTACT_DETAILS_CACHE_RECEIVE ]: ( state, { cacheData } ) => ( {
			...state,
			_contactDetailsCache: cacheData,
		} ),
		[ DOMAIN_MANAGEMENT_CONTACT_DETAILS_CACHE_UPDATE ]: ( state, { cacheData } ) =>
			merge( {}, state, { _contactDetailsCache: cacheData } ),
